...
static class MyTypeConverter {
    ...
    static MyTypeConverter() {
        ...
        RegistStringConverter(v => nuint.Parse(v, NumberStyles.Integer | NumberStyles.AllowThousands), value => value.ToString());
    ...
    private static void RegistStringConverter<T>(Func<string, T> parser, Func<T, string> formatter) where T : struct {
        ...
        // nullable分登録
        DefaultConverter<string, T?>.converter = x => string.IsNullOrEmpty(x) ? null : parser(x);
